...
export function CrmStatCard({
  ...
  return (
    <Link
      href={href}
      className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:border-sky-300 hover:shadow-md"
    >
      ...
      <ArrowUpRight className="absolute right-3 top-3 h-4 w-4 text-slate-300 opacity-0 transition group-hover:opacity-100" />
    </Link>
  );
}
...
export function CrmRevenueBreakdown({ rows, title = "Revenue mix (demo)" }: { rows: BarRow[]; title?: string }) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      ...
      <p className="mt-1 text-xs text-slate-500">Illustrative split for this demo tenant — connect ledger exports for live figures.</p>
...
export function CrmActivityHeatmap({ title = "Activity heat (demo)" }: { title?: string }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      <h2 className="text-sm font-semibold text-slate-900">{title}</h2>
      <p className="mt-1 text-xs text-slate-500">Synthetic intensity grid — replace with audit log timestamps in production.</p>
      <div className="mt-4 overflow-x-auto">
        <div className="inline-block min-w-[280px]">
          <div className="grid grid-cols-8 gap-1 text-[10px] font-semibold uppercase text-slate-400">
            <div />
            {days.map((d) => (
            ...
            {slots.map((slot, ri) => (
              <Fragment key={slot}>
                <div className="flex items-center text-[10px] font-semibold normal-case text-slate-500">{slot}</div>
                {days.map((d, ci) => {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <h2 className="text-sm font-semibold text-slate-900">{title}</h2>
        <Link href={viewHref} className="text-xs font-semibold text-sky-600 hover:underline">
          View all
        </Link>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((r) => (
              <tr key={r.id} className="bg-white hover:bg-slate-50/80">
                ...
                <td className="max-w-[180px] truncate px-4 py-3 font-medium text-slate-900">{r.org}</td>
                <td className="whitespace-nowrap px-4 py-3 text-slate-600">{r.rep}</td>
                <td className="whitespace-nowrap px-4 py-3 text-slate-600">{r.paidOn}</td>
                <td className="whitespace-nowrap px-4 py-3 font-semibold text-slate-900">{r.amount}</td>
