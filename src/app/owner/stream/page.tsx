...
export default async function OwnerStreamPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return (
    <div className="space-y-6">
      <CrmTopBar
        ...
        subtitle="Every consequential change across members, payments, claims, logistics, and HR is captured for owners and auditors. Filters mirror Pol360-style operational reviews."
      />
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-white shadow-sm">
        <table className="min-w-full divide-y divide-border text-sm">
          ...
          <tbody className="divide-y divide-border">
            {logs.map((l) => (
              <tr key={l.id} className="group hover:bg-[var(--background)]/80">
                ...
                <td className="px-4 py-3 text-xs text-foreground">
                  <div className="flex items-start justify-between gap-3">
                    <span>{l.summary}</span>
                    <span className="hidden text-[10px] font-semibold uppercase tracking-wide text-brand opacity-0 transition group-hover:inline group-hover:opacity-100">
                      {l.entityType}
